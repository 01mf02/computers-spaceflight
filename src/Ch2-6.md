...
those systems, the Apollo software effort helped \newpageon{41} provide
...
solution are among the goals of software engineering. In \newpageon{42} the
...
\newpageon{43} specification. In most cases, a team from the software
...
listing**^[83](Source2.html)^**. The tape \newpageon{44} containing the
...
\newpageon{46} computer systems and real-time systems is the fact that in
...
descent**^[106](Source2.html)^**. The restarts \newpageon{47} caused a
...
\newpageon{48} release of the program for rope manufacture was dependent on
...
\newpageon{49} additional problems. Thus, NASA's contract managers had to
...
orbit \newpageon{50} rendezvous but actually minimal enough to be
...
\newpageon{51} computer calculated the time for the de-orbit burn that
...
\newpageon{52} Even while the Task Force was investigating, Mueller took
...
\newpageon{53} executed, and *(e)* more programmers do not mean faster
