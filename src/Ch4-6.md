...
making computers do what we want"^[158](#source4)^, Flight
trainer Frank Hughes also remarked that "the PASS doesn't do anything
for us"^[159](#source4)^, noting that such practical items as
...
found the "crew\pagebreakon{123}
...
be"^[160](#source4)^. One statistic that supports his perception
is that the 13,000 keystrokes used in a week-long lunar mission are
matched by a Shuttle crew in a 58-hour flight^[161](#source4)^.
...
\pagebreakon{124} have to be displayed together on a screen because of the
...
execute key^[162](#source4)^. Young does that as well, but for
...
this situation "prone to error"^[163](#source4)^.
...
the present version with only one engine out^[164](#source4)^.
