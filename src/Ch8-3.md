...
the development of detailed documentation^[69](#source8)^.
...
so did the Orbiting Solar Observatory^[70](#source8)^. These
...
California, to track the missions^[71](#source8)^. Unlike earth
...
Canberra^[72](#source8)^. The stations were collectively named
...
machines, which were small computers^[73](#source8)^.
...
Scientific Data Systems, a new company^[74](#source8)^.
...
1958 to use with Pioneer 3 and 4^[75](#source8)^. Early Ranger
...
received^[77](#source8)^. By this time the Deep Space
...
eliminating the redundant capability^[78](#source8)^. As a
...
centers stationed at JPL^[79](#source8)^. On December 24, 1963,
JPL's director William Pickering formally established the Deep Space
Network^[80](#source8)^. Managed by William H. Bayley, with
Eberhardt Rechtin as technical head, it would serve all of NASA, just
like NASCOM did from Goddard^[81](#source8)^.
...
centralized computer center^[82](#source8)^. The combination of
...
stations^[84](#source8)^. Both the Lunar Orbiter and Surveyor
...
data were processed with these systems^[85](#source8)^. Langley
...
another layer of backup^[86](#source8)^. Later upgraded to a
...
machine time^[89](#source8)^. The primary functions of the 7094
...
Operations Facility computers^[90](#source8)^.
...
spacecraft) or a Surveyor but not both^[91](#source8)^. Surveyor
...
driving strip chart recorders^[92](#source8)^. More assistance
came during the Mariner Project when engineers realized the UNIVAC 1218
computers used in preflight testing of the spacecraft could also do
engineering telemetry analysis^[93](#source8)^. This was not
...
1983^[94](#source8)^. Each 360 had 1 megabyte of primary core
storage and 2 megabytes of LCS, half that of an Apollo-configured
machine^[95](#source8)^. Two of the 360s controlled missions as
...
navigational computations^[96](#source8)^. But the gift from
...
for Houston, but not necessarily for us"^[97](#source8)^.
...
machine, but RTOS could not handle it^[98](#source8)^. A crisis
...
once every 20 hours^[99](#source8)^. Still, something had to be
...
support for Mariner Mars 1971^[100](#source8)^. Incoming
...
tested at JPL and the software was in place. System 360s controlled the
landers^[102](#source8)^. At peak, 700 controllers \pagebreakon{265}
...
group)^[103](#source8)^. Facing dual Voyager missions and
...
implement this directive^[105](#source8)^.
...
after upgrades)^[106](#source8)^. These computers are arranged
...
rule rather than the exception^[107](#source8)^. Key to the
...
jobs alone^[111](#source8)^. In microcosm, this is the "outside
...
"minimal higher order language," specifically a hopefully
...
its Denver plant. These were technologically more advanced than the JPL
computers and could not be trusted to produce directly transportable
software^[112](#source8)^. The idea worked, but Martin admitted
that the requirement for delivering mission support software 10 months
before the flight provided strong motivation^[113](#source8)^.
...
of documents^[114](#source8)^. Based on structured programming
