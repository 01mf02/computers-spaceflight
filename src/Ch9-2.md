...
other functions in a crew trainer^[3](#source9)^. For the
...
flight^[4](#source9)^. Even the X-15 had a long series of
...
displays^[5](#source9)^. In addition to this primary trainer, a
...
and entry training^[6](#source9)^. Additionally, Langley built a
...
techniques^[7](#source9)^. The types and number of inputs and
...
computer as an interface^[9](#source9)^. Singer held the
...
resulted in a push on the astronaut's back, which increased from 0.707$g$
to 1$g$. Engine cutoff and weightless flight could be suggested by
returning the spacecraft to its original position, giving a feeling of
maximum comfort to the crew^[10](#source9)^. Negative $g$s could
...
techniques^[11](#source9)^. Still, the Apollo program
...
moon^[12](#source9)^. Another lunar landing simulator used a jet
...
training time of 29,967 hours^[13](#source9)^. These simulators
...
several computers^[14](#source9)^. Honeywell won a \$4.2 million
contract on July 21, 1966 to supply DDP-224 computers for the
complexes^[15](#source9)^. Singer--Link was again the contractor
...
could be stored^[16](#source9)^. Later, a third and fourth
...
complex^[17](#source9)^. The two types of simulators and the
...
to 200 hardware persons^[19](#source9)^. Over 350,000 words of
...
hardware^[20](#source9)^. This meant that the major components
...
simulations lagged^[21](#source9)^. The programmers had to take
...
at the time^[22](#source9)^. As a result of this study and the
...
of using a DDP-224 to simulate the AGC^[23](#source9)^. Goeckler
...
with Raney's interpretive subroutines^[24](#source9)^.
...
was unworkable^[25](#source9)^. In desperation, NASA approved
...
response^[26](#source9)^. Singer, which by then had converted
...
them^[27](#source9)^. Shuttle program director Robert F.
...
design^[28](#source9)^. Singer considered the requirements and
...
fixed-base simulator replicates the four crew stations on
the flight\pagebreakon{279}
...
directors for the mainframes^[29](#source9)^. A motion-base
...
running^[30](#source9)^. In the early 1980s it was scrapped, and
...
be interpretively simulated, as the AGC was^[31](#source9)^.
...
machines had over the AGC^[32](#source9)^. Functional simulation
...
Simulator using the rest^[33](#source9)^. The mass memory unit
...
participating computer systems^[34](#source9)^. This is about
...
software to match a new one^[35](#source9)^. Singer wrote the
real-time operating system used on the Perkin-Elmer
machines^[36](#source9)^. Despite the large number of
...
products^[37](#source9)^. In 1980, NASA's Robert Ernull, with
...
so full any modifications caused a crisis^[38](#source9)^.
...
vehicles^[39](#source9)^. That device led conceptually to a more
...
support^[41](#source9)^. One system, the SMK-23, modeled moving
vehicles such as the lunar rover, providing television window views
inside a closed control cockpit^[42](#source9)^. Besides this
...
nearly 1,750 black boxes weighing 6,000 pounds^[45](#source9)^.
...
software written for equipment under their\pagebreakon{286}
...
Kennedy^[47](#source9)^. SAIL operators can monitor tests from
...
is being prepared for flight^[48](#source9)^.
...
functions^[49](#source9)^. Separate computers generate the
...
without unmanned development flights, the SAIL's importance cannot be
