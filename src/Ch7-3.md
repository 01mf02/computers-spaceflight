...
countdown in order to achieve that many firings^[39](#source7)^.
...
reusable^[40](#source7)^. Even with this reduction, nearly
46,000 measurements have to be made and monitored in real time in the
process of preparing a Shuttle for launch^[41](#source7)^.
Clearly, there was no way NASA could do the Shuttle checkout with Apollo
concepts^[42](#source7)^. As Henry Paul, who headed the Launch
Processing System development for NASA, said, "Automation...becomes a
requirement for operations, not an elective"^[43](#source7)^.
...
Saturn countdown was 85% automated^[44](#source7)^. Building the
...
computing machinery, was a remarkable effort^[45](#source7)^.
...
savings^[46](#source7)^. In March 1972, NASA selected the solid
...
consideration^[47](#source7)^. Since existing facilities could
...
concepts of the checkout system^[48](#source7)^. Some hinted at
...
engineers^[49](#source7)^. Kennedy's own early study, based on
...
Houston^[50](#source7)^. Meanwhile, remnants of the old ACE
...
flight^[51](#source7)^. Individual BIC, for "built-in checkout,"
...
thence to the Shuttle^[52](#source7)^. General Electric built a
...
spacecraft^[53](#source7)^. The system was similar to earlier
...
vague^[54](#source7)^. Most likely, the engineers in the
...
his support clear to his engineers in 1972^[55](#source7)^.
...
put into place^[59](#source7)^. Several important advantages
...
overkill^[60](#source7)^. Second, a distributed system would
...
console^[61](#source7)^. Third, parallel testing could be
done^[62](#source7)^. A mainframe would have to be inordinately
...
individual"^[64](#source7)^. Thompson judged Kennedy's to be the
...
freeze of the design^[67](#source7)^.
...
ATOLL in concept but better and easier to use^[68](#source7)^.
...
Corporation^[72](#source7)^. The initial \$11.5 million contract
ran from May 1974 to March 1979^[73](#source7)^. This contract
...
believed it fulfilled the requirements^[74](#source7)^. IBM used
...
monitor activities^[75](#source7)^.

\pagebreakon{228} By winning the software contract first, IBM found itself in
...
hardware contracts^[76](#source7)^. According to Byrne, IBM was
...
public^[77](#source7)^. Three companies made the final round of
bids on the minicomputers: Prime, Varian Data Machines, and a small new
company called Modular Computers, Inc^[78](#source7)^. Design
...
construction^[79](#source7)^.) Because of this, many thought
Prime had the contract won, but it was edged out by Modular Computers,
much to the surprise of Byrne and Walton^[80](#source7)^.
...
applications development was delivered^[81](#source7)^.
...
personnel for that Subsystem during 1976--1977^[82](#source7)^.
...
the "cornerstone of the system"^[83](#source7)^. The biggest
...
accessed by both^[84](#source7)^. ACE stations also used common
...
built^[85](#source7)^. Memory chips are made of negative
...
error-correcting code used in transmissions^[86](#source7)^.
...
memory^[87](#source7)^. The 64K main memory has the first 1K
...
area^[88](#source7)^. Data can move through the temporary
...
request^[90](#source7)^. In this way, when one machine is
...
that it would be a single point of failure^[92](#source7)^.
...
components^[93](#source7)^. However, the biggest problem in a
...
specifically to work on the problem^[94](#source7)^.
...
"big" signal losses^[95](#source7)^. Individual bits are checked
...
generated by software on read-only memories^[96](#source7)^.
...
\$10,000 bonus and an award^[98](#source7)^. The buffer itself
...
number four used for secure operations^[100](#source7)^. Firing
room two has three buffers to facilitate multiple parallel software
development^[101](#source7)^. Operations firing rooms normally
...
are kept active as a further backup^[103](#source7)^. Each
...
CDS^[104](#source7)^. Early in the program each engineer had his
...
replaced^[105](#source7)^. Loading an entire firing room through
...
System^[106](#source7)^. Some of the resulting concepts make the
...
using the keyboard^[107](#source7)^. This concept antedates by
...
of GOAL applications^[108](#source7)^. As a further convenience,
...
other devices^[109](#source7)^. Hardware interface modules at
...
matter^[110](#source7)^. Control logic is a subset of GOAL used
...
memory^[111](#source7)^. One hundred seventy-two disk drives are
...
needs of the Launch Processing System^[112](#source7)^. Most
...
tests^[113](#source7)^. Firing room engineers can play back
tests or other data directly to their firing room consoles for problem
resolution or trend analysis^[114](#source7)^. At first, the RPS
...
connections^[115](#source7)^. This situation was corrected
...
needs^[116](#source7)^. They essentially built their own
tools^[117](#source7)^. That policy continues in the
Applications and Simulations Branch, which helps the engineers refine
their test requirements^[118](#source7)^.
...
up to less than 700K words^[119](#source7)^. Despite the early
...
ATOLL or GOAL programs^[120](#source7)^. In instructing a
...
group^[121](#source7)^. Medlock and his team of civil servants
...
program)^[122](#source7)^. IBM designed GOAL's compiler to
disallow any undefined branches or procedures, making it more strict
than FORTRAN compilers^[123](#source7)^. GOAL is highly flexible
and permits engineers to decide for themselves the degree of interaction
required to do a test^[124](#source7)^. GOAL programs are run
within the computers in time slices of 10
milliseconds^[125](#source7)^.

\pagebreakon{237} When an engineer is developing a GOAL procedure, he writes
...
as fueling systems and external power^[126](#source7)^. However,
...
Johnson Space Center^[127](#source7)^. In addition to that
...
anomaly^[129](#source7)^. Control logic thus makes parallel
operations safe.
...
automatically^[132](#source7)^. With thousands of GOAL
...
for all the customers^[133](#source7)^.
...
existing contracts and computers^[134](#source7)^. Accordingly,
...
simulations kept on the CDS^[135](#source7)^.
...
involved^[136](#source7)^. However, in late 1983, NASA awarded
...
values occur^[137](#source7)^. If his inquiry bears fruit,
...
1966^[138](#source7)^. Originally designed as a Titan III launch
...
separated^[139](#source7)^. With the commissioning of the
